...
TOOL_IMPLS = {
    ...
    "get_semester": lambda args: get_semester(
        args["program_id"], args["year"], args["term"]
    ),
    "get_course": lambda args: get_course(args["course_code"]),
    "list_semesters_for_course": lambda args: list_semesters_for_course(
        args["course_code"]
    ),
}

# -------------------------
# 3. Ollama-as-OpenAI client
# -------------------------
# Ollama exposes an OpenAI-compatible API at http://localhost:11434/v1

client = OpenAI(
    base_url=os.getenv("OLLAMA_OPENAI_BASE_URL", "http://localhost:11434/v1"),
    api_key=os.getenv("OLLAMA_OPENAI_API_KEY", "ollama"),  # any non-empty string
)

OLLAMA_MODEL = os.getenv("OLLAMA_OPENAI_MODEL", "gpt-oss:20b")  # must support tools
...
if __name__ == "__main__":
    print("Curriculum assistant (Ollama + OpenAI-compatible tools). Ask a question (Ctrl+C to exit).")
    while True:
        try:
            q = input("\nYou: ")
