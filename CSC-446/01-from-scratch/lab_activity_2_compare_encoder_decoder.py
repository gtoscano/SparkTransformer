...
# - Generates text with GPT-2 under different decoding controls
# - Reports speed, context size, controllability, and prints a comparison table
...
# Measure single-example inference latency (CPU vs GPU friendly)
@torch.no_grad()
...
# Context size
gpt2_context = int(getattr(gpt2_model.config, "n_positions", getattr(gpt2_model.config, "max_position_embeddings", 1024)))
...
def timed_generate(prompt: str, max_new_tokens=80, **gen_kwargs) -> Dict[str, Any]:
    ...
    # warmup
    _ = gpt2_model.generate(input_ids=input_ids, max_new_tokens=4, do_sample=False)
    t1 = time.time()
